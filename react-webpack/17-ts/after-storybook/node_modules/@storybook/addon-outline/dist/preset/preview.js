...
var clearStyle = (selector) => {
  const element = import_global.default.document.getElementById(selector);
  if (element && element.parentElement) {
    element.parentElement.removeChild(element);
  }
};
var addOutlineStyles = (selector, css) => {
  const existingStyle = import_global.default.document.getElementById(selector);
  if (existingStyle) {
    if (existingStyle.innerHTML !== css) {
      existingStyle.innerHTML = css;
    }
  } else {
    const style = import_global.default.document.createElement("style");
...
var withOutline = (StoryFn, context) => {
  const { globals: globals2 } = context;
  const isActive = globals2[PARAM_KEY] === true;
  const isInDocs = context.viewMode === "docs";
  const outlineStyles = (0, import_addons.useMemo)(() => {
