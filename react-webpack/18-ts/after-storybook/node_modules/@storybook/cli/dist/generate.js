...
var import_read_pkg_up3 = require("read-pkg-up");
var import_node_logger8 = require("@storybook/node-logger");

// src/initiate.ts
var import_update_notifier = require("update-notifier");
var import_chalk15 = __toESM(require("chalk"));
var import_prompts3 = __toESM(require("prompts"));
var import_telemetry = require("@storybook/telemetry");

// src/project_types.ts
var import_semver = require("@storybook/semver");
function ltMajor(versionRange, major) {
  return (0, import_semver.validRange)(versionRange) && (0, import_semver.minVersion)(versionRange).major < major;
}
...
var import_semver2 = require("@storybook/semver");
var import_strip_json_comments = __toESM(require("strip-json-comments"));

// src/dirs.ts
var import_path = require("path");
function getBaseDir() {
  return (0, import_path.dirname)(require.resolve("@storybook/cli/package.json"));
}
...
function getBowerJson() {
  const bowerJsonPath = import_path2.default.resolve("bower.json");
  if (!import_fs.default.existsSync(bowerJsonPath)) {
    return false;
  }
  const jsonContent = import_fs.default.readFileSync(bowerJsonPath, "utf8");
  return JSON.parse(jsonContent);
}
function readFileAsJson(jsonPath, allowComments) {
  const filePath = import_path2.default.resolve(jsonPath);
  if (!import_fs.default.existsSync(filePath)) {
    return false;
  }
  const fileContent = import_fs.default.readFileSync(filePath, "utf8");
  const jsonContent = allowComments ? (0, import_strip_json_comments.default)(fileContent) : fileContent;
  try {
    return JSON.parse(jsonContent);
  } catch (e) {
    logger.error(import_chalk.default.red(`Invalid json in file: ${filePath}`));
    throw e;
  }
}
var writeFileAsJson = (jsonPath, content) => {
  ...
  import_fs.default.writeFileSync(filePath, `${JSON.stringify(content, null, 2)}
`);
  return true;
};
...
function codeLog(codeLines, leftPadAmount) {
  ...
  const finalResult = newLines.map((line) => {
    ...
    newLine = getChars(" ", leftPadAmount || 2) + import_chalk.default.inverse(` ${newLine} `);
    return newLine;
  }).join("\n");
...
async function getBabelDependencies(packageManager, packageJson) {
  ...
  if (!babelCoreVersion) {
    ...
    if ((0, import_semver2.satisfies)(latestCompatibleBabelVersion, "^6.0.0")) {
...
function copyTemplate(templateRoot) {
  const templateDir = import_path2.default.resolve(templateRoot, `template-csf/`);
  if (!import_fs.default.existsSync(templateDir)) {
    throw new Error(`Couldn't find template dir`);
  }
  import_fs_extra.default.copySync(templateDir, ".", { overwrite: true });
}
async function copyComponents(renderer, language) {
  ...
  const componentsPath = async () => {
    ...
    const assetsRoot = (0, import_path2.join)(baseDir, "rendererAssets");
    const assetsRenderer = (0, import_path2.join)(assetsRoot, renderer);
    const assetsLanguage = (0, import_path2.join)(assetsRenderer, languageFolderMapping[language]);
    if (await import_fs_extra.default.pathExists(assetsLanguage)) {
      return assetsLanguage;
    }
    const assetsJS = (0, import_path2.join)(assetsRenderer, languageFolderMapping.javascript);
    if (await import_fs_extra.default.pathExists(assetsJS)) {
      return assetsJS;
    }
    if (await import_fs_extra.default.pathExists(assetsRenderer)) {
  ...
  await import_fs_extra.default.copy(await componentsPath(), destinationPath, { overwrite: true });
  await import_fs_extra.default.copy((0, import_path2.join)(getBaseDir(), "rendererAssets/common"), destinationPath, {
    overwrite: true
  });
}
...
var hasDependency = (packageJson, name, matcher) => {
  var _a2, _b;
  const version = ((_a2 = packageJson.dependencies) == null ? void 0 : _a2[name]) || ((_b = packageJson.devDependencies) == null ? void 0 : _b[name]);
...
var hasPeerDependency = (packageJson, name, matcher) => {
  var _a2;
  const version = (_a2 = packageJson.peerDependencies) == null ? void 0 : _a2[name];
  if (version && typeof matcher === "function") {
    return matcher(version);
  }
  return !!version;
...
var getFrameworkPreset = (packageJson, framework) => {
  ...
  if (Array.isArray(files) && files.length > 0) {
    matcher.files = files.map((name) => import_fs2.default.existsSync(import_path3.default.join(process.cwd(), name)));
  }
...
function detectBuilder(packageManager) {
  const viteConfig = import_find_up.default.sync(viteConfigFiles);
  if (viteConfig) {
...
var import_semver5 = __toESM(require("@storybook/semver"));
...
function getDefaultProjectName(angularJson) {
  ...
  if (projects == null ? void 0 : projects.storybook) {
    return "storybook";
  }
  if (defaultProject) {
...
async function getBaseTsConfigName() {
  return await (0, import_fs_extra2.pathExists)("./tsconfig.base.json") ? "tsconfig.base.json" : "tsconfig.json";
}
...
async function configureMain({
  ...
  const prefix = await import_fs_extra3.default.pathExists("./src") ? "../src" : "../stories";
  const config = {
...
var frameworkToPreviewParts = {
  angular: {
    prefix: import_ts_dedent.dedent`
...
async function configurePreview(framework) {
  ...
  if (await import_fs_extra3.default.pathExists(previewPath)) {
    return;
  }
  const preview = import_ts_dedent.dedent`
    ${prefix}
  ...
  await import_fs_extra3.default.writeFile(previewPath, preview, { encoding: "utf8" });
}
...
var import_semver4 = __toESM(require("@storybook/semver"));

// src/js-package-manager/JsPackageManager.ts
var import_chalk2 = __toESM(require("chalk"));
var import_semver3 = require("@storybook/semver");
...
var JsPackageManager = class {
  ...
  packageJsonPath() {
    return this.cwd ? import_path4.default.resolve(this.cwd, "package.json") : import_path4.default.resolve("package.json");
  }
  readPackageJson() {
    ...
    const jsonContent = import_fs3.default.readFileSync(packageJsonPath, "utf8");
    return JSON.parse(jsonContent);
  }
  writePackageJson(packageJson) {
    const content = `${JSON.stringify(packageJson, null, 2)}
`;
    import_fs3.default.writeFileSync(this.packageJsonPath(), content, "utf8");
  }
  retrievePackageJson() {
    let packageJson;
    try {
  ...
  addStorybookCommandInScripts(options) {
    ...
    const storybookCmd = (options == null ? void 0 : options.staticFolder) ? `npx storybook dev -p ${sbPort} -s ${options.staticFolder}` : `npx storybook dev -p ${sbPort}`;
    const buildStorybookCmd = (options == null ? void 0 : options.staticFolder) ? `npx storybook build -s ${options.staticFolder}` : `npx storybook build`;
  ...
  addESLintConfig() {
    ...
    this.writePackageJson({
      ...
      eslintConfig: {
        ...
        overrides: [
          ...((_a2 = packageJson.eslintConfig) == null ? void 0 : _a2.overrides) || [],
          {
            files: ["**/*.stories.*"],
            rules: {
  ...
  executeCommand(command2, args, stdio) {
    const commandResult = (0, import_cross_spawn.sync)(command2, args, {
      cwd: this.cwd,
      stdio: stdio ?? "pipe",
      encoding: "utf-8",
      shell: true
...
var NPMProxy = class extends JsPackageManager {
  ...
  needsLegacyPeerDeps(version) {
    return import_semver4.default.gte(version, "7.0.0") && !this.hasLegacyPeerDeps();
  }
  getInstallArgs() {
    if (!this.installArgs) {
...
var JsPackageManagerFactory = class {
  static getPackageManager(forceNpmUsage = false, cwd) {
    ...
    const hasYarnLockFile = (0, import_find_up2.sync)("yarn.lock", { cwd });
...
var generateStorybookBabelConfig = async ({ target }) => {
  import_node_logger.logger.info(`Generating the storybook default babel config at ${target}`);
  ...
  const location = import_path5.default.join(target, fileName);
  const exists = await (0, import_fs_extra4.access)(location).then(
    () => true,
    () => false
  );
  if (exists) {
    ...
    if (overwrite === false) {
      import_node_logger.logger.warn(`Cancelled, babel config file was NOT written to file-system.`);
      return;
    }
  }
  import_node_logger.logger.info(`Writing file to ${location}`);
  await (0, import_fs_extra4.writeFile)(location, contents);
};
...
async function baseGenerator(packageManager, npmOptions, { language, builder = "webpack5" /* Webpack5 */, pnp, commonJs }, renderer, options = defaultOptions) {
  ...
  const files = await import_fs_extra5.default.readdir(process.cwd());
  ...
  await import_fs_extra5.default.ensureDir("./.storybook");
  await configureMain({
  ...
  if (frameworkPackages.find((pkg2) => pkg2.match(/^@storybook\/.*-vite$/))) {
    const previewHead = (0, import_ts_dedent2.dedent)(_a || (_a = __template(["\n      <script>\n        window.global = window;\n      <\/script>\n    "])));
    await import_fs_extra5.default.writeFile(`.storybook/preview-head.html`, previewHead, { encoding: "utf8" });
  }
...
var generator = async (packageManager, npmOptions, options) => {
  var _a2;
  checkForProjects();
  const angularVersion = (_a2 = import_semver5.default.coerce(
    packageManager.retrievePackageJson().dependencies["@angular/core"]
  )) == null ? void 0 : _a2.version;
  const isWebpack5 = import_semver5.default.gte(angularVersion, "12.0.0");
  ...
  const templateDir = (0, import_path6.join)(getBaseDir(), "templates", "angular");
  copyTemplate(templateDir);
  editAngularAppTsConfig();
  editStorybookTsConfig(import_path6.default.resolve("./.storybook/tsconfig.json"));
...
var generator2 = async (packageManager, npmOptions, options) => {
  ...
  const templateDir = (0, import_path7.join)(getBaseDir(), "templates", "aurelia");
  copyTemplate(templateDir);
};
...
var generator5 = async (packageManager, npmOptions, installServer) => {
  const dirname2 = import_shelljs.default.ls("-d", "ios/*.xcodeproj").stdout;
  if (dirname2) {
    const projectName = dirname2.slice("ios/".length, dirname2.length - ".xcodeproj".length - 1);
    if (projectName) {
      import_shelljs.default.sed("-i", "%APP_NAME%", projectName, "storybook/index.js");
    } else {
      paddedLog(
  ...
  const templateDir = (0, import_path8.join)(getBaseDir(), "templates", "react-native");
  copyTemplate(templateDir);
};
...
var import_semver6 = __toESM(require("@storybook/semver"));
var generator6 = async (packageManager, npmOptions, options) => {
  var _a2;
  const monorepoRootPath = import_path9.default.join(__dirname, "..", "..", "..", "..", "..", "..");
  ...
  const craVersion = (_a2 = import_semver6.default.coerce(
    packageManager.retrievePackageJson().dependencies["react-scripts"]
  )) == null ? void 0 : _a2.version;
  const isCra5 = craVersion && import_semver6.default.gte(craVersion, "5.0.0");
  ...
  await baseGenerator(packageManager, npmOptions, updatedOptions, "react", {
    extraAddons,
    extraPackages,
    staticDir: import_fs4.default.existsSync(import_path9.default.resolve("./public")) ? "public" : void 0,
    addBabel: false,
    addESLint: true,
    extraMain
  });
...
var generator18 = async (packageManager, npmOptions, options) => {
  let extraMain;
  if (import_fs_extra6.default.existsSync("./svelte.config.js")) {
    import_node_logger2.logger.info("Configuring preprocessor from 'svelte.config.js'");
    extraMain = {
      svelteOptions: { preprocess: '%%require("../svelte.config.js").preprocess%%' }
    };
  } else if (import_fs_extra6.default.existsSync("./svelte.config.cjs")) {
    import_node_logger2.logger.info("Configuring preprocessor from 'svelte.config.cjs'");
    extraMain = {
    ...
    if (packageJson.devDependencies && packageJson.devDependencies["svelte-preprocess"]) {
      import_node_logger2.logger.info("Configuring preprocessor with 'svelte-preprocess'");
      extraMain = {
...
var generator20 = async (packageManager, npmOptions, options) => {
  ...
  const templateDir = (0, import_path10.join)(getBaseDir(), "templates", "server");
  copyTemplate(templateDir);
};
...
var import_semver8 = __toESM(require("@storybook/semver"));

// src/automigrate/fixes/webpack5.ts
var import_chalk4 = __toESM(require("chalk"));
var import_ts_dedent3 = require("ts-dedent");
var import_semver7 = __toESM(require("@storybook/semver"));
...
var webpack5 = {
  ...
  async checkWebpack5Builder(packageJson) {
    var _a2;
    const { mainConfig, version: storybookVersion } = (0, import_core_common.getStorybookInfo)(packageJson);
    const storybookCoerced = storybookVersion && ((_a2 = import_semver7.default.coerce(storybookVersion)) == null ? void 0 : _a2.version);
    if (!storybookCoerced) {
      logger5.warn(import_ts_dedent3.dedent`
        ❌ Unable to determine storybook version, skipping ${import_chalk4.default.cyan("webpack5")} fix.
        🤔 Are you running automigrate from your project directory?
      `);
      return null;
    }
    if (import_semver7.default.lt(storybookCoerced, "6.3.0")) {
      logger5.warn(
    ...
    if (import_semver7.default.gte(storybookCoerced, "7.0.0")) {
      return null;
    }
    if (!mainConfig) {
    ...
    const main = await (0, import_csf_tools.readConfig)(mainConfig);
  ...
  async check({ packageManager }) {
    ...
    const webpackCoerced = (_a2 = import_semver7.default.coerce(webpackVersion)) == null ? void 0 : _a2.version;
    if (!webpackCoerced || import_semver7.default.lt(webpackCoerced, "5.0.0") || import_semver7.default.gte(webpackCoerced, "6.0.0"))
      return null;
  ...
  prompt({ webpackVersion }) {
    const webpackFormatted = import_chalk4.default.cyan(`webpack ${webpackVersion}`);
    return import_ts_dedent3.dedent`
      ...
      To run Storybook in webpack5-mode, we can install Storybook's ${import_chalk4.default.cyan(
      "@storybook/builder-webpack5"
    )} for you.

      More info: ${import_chalk4.default.yellow(
  ...
  async run({ result: { main, storybookVersion, webpackVersion }, packageManager, dryRun }) {
    ...
    if (!dryRun) {
      ...
      await (0, import_csf_tools.writeConfig)(main);
    }
  }
};
...
var cra5 = {
  ...
  async check({ packageManager }) {
    ...
    const craCoerced = (_a2 = import_semver8.default.coerce(craVersion)) == null ? void 0 : _a2.version;
    if (!craCoerced || import_semver8.default.lt(craCoerced, "5.0.0")) {
      return null;
    }
  ...
  prompt({ craVersion, ...rest }) {
    const craFormatted = import_chalk5.default.cyan(`Create React App (CRA) ${craVersion}`);
    console.log({ ...rest });
    return import_ts_dedent4.dedent`
      We've detected you are running ${craFormatted} which is powered by webpack5.
      Your Storybook's main.js files specifies webpack4, which is incompatible.

      In order to work with your version of CRA, we need to install Storybook's ${import_chalk5.default.cyan(
      "@storybook/builder-webpack5"
    )}.

      More info: ${import_chalk5.default.yellow(
...
var import_semver9 = __toESM(require("@storybook/semver"));
var angular12 = {
  id: "angular12",
  async check({ packageManager }) {
    var _a2;
    ...
    const angularCoerced = (_a2 = import_semver9.default.coerce(angularVersion)) == null ? void 0 : _a2.version;
    if (!angularCoerced || import_semver9.default.lt(angularCoerced, "12.0.0")) {
      return null;
    }
    const builderInfo = await webpack5.checkWebpack5Builder(packageJson);
  ...
  prompt({ angularVersion }) {
    const angularFormatted = import_chalk6.default.cyan(`Angular ${angularVersion}`);
    return import_ts_dedent5.dedent`
      We've detected you are running ${angularFormatted} which is powered by webpack5.
      Your Storybook's main.js files specifies webpack4, which is incompatible.

      In order to work with your version of Angular, we need to install Storybook's ${import_chalk6.default.cyan(
      "@storybook/builder-webpack5"
    )}.

      More info: ${import_chalk6.default.yellow(
...
var import_semver10 = __toESM(require("@storybook/semver"));
var vue3 = {
  id: "vue3",
  async check({ packageManager }) {
    var _a2;
    ...
    const vueCoerced = (_a2 = import_semver10.default.coerce(vueVersion)) == null ? void 0 : _a2.version;
    if (!vueCoerced || import_semver10.default.lt(vueCoerced, "3.0.0")) {
      return null;
    }
    const builderInfo = await webpack5.checkWebpack5Builder(packageJson);
  ...
  prompt({ vueVersion, storybookVersion }) {
    const vueFormatted = import_chalk7.default.cyan(`Vue ${vueVersion}`);
    const sbFormatted = import_chalk7.default.cyan(`Storybook ${storybookVersion}`);
    return import_ts_dedent6.dedent`
      We've detected you are running ${vueFormatted} with Storybook.
      ${sbFormatted} runs webpack4 by default, which is incompatible.

      In order to work with your version of Vue, we need to install Storybook's ${import_chalk7.default.cyan(
      "webpack5 builder"
    )}.

      More info: ${import_chalk7.default.yellow(
...
var import_semver11 = __toESM(require("@storybook/semver"));
var import_csf_tools2 = require("@storybook/csf-tools");
...
var mainjsFramework = {
  ...
  async check({ packageManager }) {
    var _a2;
    const packageJson = packageManager.retrievePackageJson();
    const { mainConfig, framework, version: storybookVersion } = (0, import_core_common2.getStorybookInfo)(packageJson);
    if (!mainConfig) {
      logger6.warn("Unable to find storybook main.js config, skipping");
      return null;
    }
    const storybookCoerced = storybookVersion && ((_a2 = import_semver11.default.coerce(storybookVersion)) == null ? void 0 : _a2.version);
    if (!storybookCoerced) {
      logger6.warn(import_ts_dedent7.dedent`
        ❌ Unable to determine storybook version, skipping ${import_chalk8.default.cyan("mainjsFramework")} fix.
        🤔 Are you running automigrate from your project directory?
      `);
      return null;
    }
    const main = await (0, import_csf_tools2.readConfig)(mainConfig);
    const currentFramework = main.getFieldValue(["framework"]);
    const features = main.getFieldValue(["features"]);
    if (currentFramework)
      return null;
    return (features == null ? void 0 : features.breakingChangesV7) || (features == null ? void 0 : features.storyStoreV7) || import_semver11.default.gte(storybookCoerced, "7.0.0") ? { main, framework: `@storybook/${framework}` } : null;
  },
  prompt({ framework }) {
    const frameworkFormatted = import_chalk8.default.cyan(`framework: '${framework}'`);
    return import_ts_dedent7.dedent`
      ...
      More info: ${import_chalk8.default.yellow(
  ...
  async run({ result: { main, framework }, dryRun }) {
    ...
    if (!dryRun) {
      ...
      await (0, import_csf_tools2.writeConfig)(main);
    }
  }
};
...
var import_csf_tools3 = require("@storybook/csf-tools");
...
var findEslintFile = () => {
  ...
  const unsupportedExtension = UNSUPPORTED_ESLINT_EXTENSIONS.find(
    (ext) => import_fs_extra7.default.existsSync(`${filePrefix}.${ext}`)
  );
...
var eslintPlugin = {
  ...
  async check({ packageManager }) {
    ...
    const { mainConfig } = (0, import_core_common3.getStorybookInfo)(packageJson);
    if (!mainConfig) {
    ...
    const main = await (0, import_csf_tools3.readConfig)(mainConfig);
  ...
  async run({ result: { eslintFile, unsupportedExtension }, packageManager, dryRun }) {
    ...
    const eslint = await (0, import_csf_tools3.readConfig)(eslintFile);
    ...
    if (!dryRun) {
      ...
      await (0, import_csf_tools3.writeConfig)(eslint);
    }
  }
};
...
var import_csf_tools4 = require("@storybook/csf-tools");
...
var builderVite = {
  ...
  async check({ packageManager }) {
    const packageJson = packageManager.retrievePackageJson();
    const { mainConfig } = (0, import_core_common4.getStorybookInfo)(packageJson);
    if (!mainConfig) {
      logger8.warn("Unable to find storybook main.js config");
      return null;
    }
    const main = await (0, import_csf_tools4.readConfig)(mainConfig);
    const builder = main.getFieldValue(["core", "builder"]);
    const builderName = typeof builder === "string" ? builder : builder == null ? void 0 : builder.name;
  ...
  prompt({ builder }) {
    const builderFormatted = import_chalk10.default.cyan(JSON.stringify(builder, null, 2));
    return import_ts_dedent9.dedent`
      We've detected you're using the community vite builder: ${builderFormatted}
      
      'storybook-builder-vite' is deprecated and now located at ${import_chalk10.default.cyan(
      "@storybook/builder-vite"
    )}.

      We can upgrade your project to use the new builder automatically.
      
      More info: ${import_chalk10.default.yellow(
  ...
  async run({ result: { builder, main, packageJson }, packageManager, dryRun }) {
    ...
    if (!dryRun) {
      ...
      await (0, import_csf_tools4.writeConfig)(main);
    }
  }
};
...
var npm7 = {
  ...
  prompt({ npmVersion }) {
    const npmFormatted = import_chalk11.default.cyan(`npm ${npmVersion}`);
    return import_ts_dedent10.dedent`
      ...
      More info: ${import_chalk11.default.yellow("https://github.com/storybookjs/storybook/issues/18298")}
    `;
  },
...
var import_semver12 = __toESM(require("@storybook/semver"));
...
var sbScripts = {
  ...
  async check({ packageManager }) {
    ...
    const { version: storybookVersion } = (0, import_core_common5.getStorybookInfo)(packageJson);
    const allDeps = { ...dependencies, ...devDependencies };
    const storybookCoerced = storybookVersion && ((_a2 = import_semver12.default.coerce(storybookVersion)) == null ? void 0 : _a2.version);
    if (!storybookCoerced) {
      logger9.warn(import_ts_dedent11.dedent`
        ❌ Unable to determine storybook version, skipping ${import_chalk12.default.cyan("sb-scripts")} fix.
        🤔 Are you running automigrate from your project directory?
      `);
      return null;
    }
    ...
    return import_semver12.default.gte(storybookCoerced, "7.0.0") ? { packageJson, storybookScripts, storybookVersion } : null;
  },
  prompt({ storybookVersion }) {
    const sbFormatted = import_chalk12.default.cyan(`Storybook ${storybookVersion}`);
    const explanationMessage = [
      `Starting in Storybook 7, the ${import_chalk12.default.yellow("start-storybook")} and ${import_chalk12.default.yellow(
        "build-storybook"
      )} binaries have changed to ${import_chalk12.default.magenta("storybook dev")} and ${import_chalk12.default.magenta(
        "storybook build"
      )} respectively.`,
      `In order to work with ${sbFormatted}, Storybook's ${import_chalk12.default.magenta(
        "storybook"
    ...
    return [
      ...
      explanationMessage,
      `More info: ${import_chalk12.default.yellow(
  ...
  async run({ result: { storybookScripts, packageJson }, packageManager, dryRun }) {
    ...
    if (!dryRun && Object.keys(storybookScripts.official).length > 0) {
      const message = [
        ...
        import_chalk12.default.yellow(JSON.stringify(storybookScripts.official, null, 2))
      ].join("\n");
      logger9.log(message);
      logger9.log();
    ...
    if (!dryRun && Object.keys(storybookScripts.custom).length > 0) {
      const message = [
        ...
        import_chalk12.default.yellow(JSON.stringify(storybookScripts.custom, null, 2)),
        "\n",
        `Please manually migrate the ones applicable and use the documentation below for reference: ${import_chalk12.default.yellow(
          "https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#start-storybook--build-storybook-binaries-removed"
        )}`
      ].join("\n");
      logger9.log(message);
...
var import_ts_dedent12 = __toESM(require("ts-dedent"));
var import_semver13 = __toESM(require("@storybook/semver"));
var import_csf_tools5 = require("@storybook/csf-tools");
...
var getBuilder = (builder) => {
  ...
  return (builder == null ? void 0 : builder.name.includes("vite")) ? "vite" : "webpack5";
};
...
var newFrameworks = {
  ...
  async check({ packageManager }) {
    ...
    const packageJson = packageManager.retrievePackageJson();
    const allDeps = { ...packageJson.dependencies, ...packageJson.devDependencies };
    const { mainConfig, version: storybookVersion, framework } = (0, import_core_common6.getStorybookInfo)(packageJson);
    if (!mainConfig) {
      logger10.warn("Unable to find storybook main.js config, skipping");
      return null;
    }
    const storybookCoerced = storybookVersion && ((_a2 = import_semver13.default.coerce(storybookVersion)) == null ? void 0 : _a2.version);
    if (!storybookCoerced) {
      logger10.warn(import_ts_dedent12.default`
        ❌ Unable to determine storybook version, skipping ${import_chalk13.default.cyan("newFrameworks")} fix.
        🤔 Are you running automigrate from your project directory?
      `);
      return null;
    }
    if (!import_semver13.default.gte(storybookCoerced, "7.0.0")) {
      return null;
    }
    const main = await (0, import_csf_tools5.readConfig)(mainConfig);
    ...
    if (allDeps.vite && import_semver13.default.lt(import_semver13.default.coerce(allDeps.vite).version, "3.0.0")) {
      logger10.warn(import_ts_dedent12.default`
        ❌ Detected Vite ${allDeps.vite}, which is unsupported in Storybook 7.0, so the ${import_chalk13.default.cyan(
        "newFrameworks"
      )} fix will be skipped.
  ...
  prompt() {
    return import_ts_dedent12.default`
      ...
      To learn more about the framework field, see: ${import_chalk13.default.yellow(
      "https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#framework-field-mandatory"
    )}

      ${import_chalk13.default.underline(import_chalk13.default.bold(import_chalk13.default.cyan("Webpack4 users")))}
      ...
      To learn more about migrating from Webpack4, see: ${import_chalk13.default.yellow(
  ...
  async run({
    ...
    if (!dryRun) {
      ...
      if (currentCore == null ? void 0 : currentCore.builder) {
      ...
      await (0, import_csf_tools5.writeConfig)(main);
    }
  }
};
...
var automigrate = async ({ fixId, dryRun, yes } = {}) => {
  ...
  for (let i = 0; i < filtered.length; i += 1) {
    ...
    if (result) {
      logger11.info(`\u{1F50E} found a '${import_chalk14.default.cyan(f.id)}' migration:`);
      logger11.info();
      const message = f.prompt(result);
      logger11.info(
        (0, import_boxen.default)(message, { borderStyle: "round", padding: 1, borderColor: "#F1618C" })
      );
      let runAnswer;
      if (dryRun) {
        runAnswer = { fix: false };
        ...
        runAnswer = await (0, import_prompts2.default)({
          type: "confirm",
          name: "fix",
          message: `Do you want to run the '${import_chalk14.default.cyan(f.id)}' migration on your project?`
        });
      }
      if (runAnswer.fix) {
        try {
          await f.run({ result, packageManager, dryRun });
          logger11.info(`\u2705 ran ${import_chalk14.default.cyan(f.id)} migration`);
        } catch (error) {
          logger11.info(`\u274C error when running ${import_chalk14.default.cyan(f.id)} migration:`);
          logger11.info(error);
          logger11.info();
        }
      } else {
        logger11.info(`Skipping the ${import_chalk14.default.cyan(f.id)} migration.`);
        logger11.info();
        logger11.info(
          `If you change your mind, run '${import_chalk14.default.cyan("npx storybook@next automigrate")}'`
        );
      }
    }
  }
...
var installStorybook = (projectType, packageManager, options) => {
  ...
  const runGenerator = async () => {
    switch (projectType) {
      ...
      case "REACT_NATIVE" /* REACT_NATIVE */: {
        return (options.yes ? Promise.resolve({ server: true }) : (0, import_prompts3.default)([
          {
            type: "confirm",
            name: "server",
  ...
  return runGenerator().catch((ex) => {
    logger12.error(`
     ${import_chalk15.default.red(ex.stack)}`);
    process.exit(1);
  });
};
var projectTypeInquirer = async (options, packageManager) => {
  const manualAnswer = options.yes ? true : await (0, import_prompts3.default)([
    {
      type: "confirm",
      name: "manual",
  ...
  if (manualAnswer !== true && manualAnswer.manual) {
    const frameworkAnswer = await (0, import_prompts3.default)([
      {
        type: "select",
...
async function initiate(options, pkg2) {
  ...
  logger12.log(import_chalk15.default.inverse(`
 ${welcomeMessage} 
`));
  if (!options.disableTelemetry) {
    (0, import_telemetry.telemetry)("init");
  }
  new import_update_notifier.UpdateNotifier({
    pkg: pkg2,
...
`);
  ...
  logger12.log("\nFor more information visit:", import_chalk15.default.cyan("https://storybook.js.org"));
  if (projectType === "REACT_NATIVE" /* REACT_NATIVE */) {
    const REACT_NATIVE_REPO = "https://github.com/storybookjs/react-native";
    logger12.log();
    logger12.log(import_chalk15.default.red("NOTE: installation is not 100% automated."));
    logger12.log(`To quickly run Storybook, replace contents of your app entry with:
`);
    codeLog(["export {default} from './storybook';"]);
    logger12.log("\n For more in information, see the github readme:\n");
    logger12.log(import_chalk15.default.cyan(REACT_NATIVE_REPO));
    logger12.log();
  }
  logger12.log();
}
...
var import_cross_spawn3 = require("cross-spawn");
var import_core_common7 = require("@storybook/core-common");
var import_csf_tools6 = require("@storybook/csf-tools");
...
var postinstallAddon = async (addonName, isOfficialAddon) => {
  ...
  if (!isOfficialAddon) {
  ...
  } else if (!import_fs5.default.existsSync(".storybook")) {
    ...
    LEGACY_CONFIGS.forEach((config) => {
      try {
        ...
        let configFile = import_path11.default.join(".storybook", `${config}.ts`);
        if (!import_fs5.default.existsSync(configFile)) {
          configFile = import_path11.default.join(".storybook", `${config}.js`);
          if (!import_fs5.default.existsSync(configFile)) {
            import_fs5.default.writeFileSync(configFile, "", "utf8");
          }
        }
        (0, import_cross_spawn3.sync)("npx", ["jscodeshift", "-t", codemod, configFile], {
          stdio: "inherit",
          shell: true
        });
        skipMsg = null;
...
async function add(addon, options) {
  ...
  const { mainConfig, version: storybookVersion } = (0, import_core_common7.getStorybookInfo)(packageJson);
  if (!mainConfig) {
    logger13.error("Unable to find storybook main.js config");
    return;
  }
  const main = await (0, import_csf_tools6.readConfig)(mainConfig);
  ...
  await (0, import_csf_tools6.writeConfig)(main);
...
// src/migrate.ts
var import_codemod = require("@storybook/codemod");
async function migrate(migration, { configDir, glob, dryRun, list, rename, logger: logger23, parser }) {
  if (list) {
    (0, import_codemod.listCodemods)().forEach((key) => logger23.log(key));
  } else if (migration) {
    await (0, import_codemod.runCodemod)(migration, { configDir, glob, dryRun, logger: logger23, rename, parser });
  } else {
...
var useLocation = async (input) => {
  await (0, import_fs_extra8.stat)(import_path12.default.resolve(input));
  if (input.match(/^http/)) {
    return [input, async () => {
    }];
  }
  const app = (0, import_express.default)();
  app.use(import_express.default.static(input));
  const port = await (0, import_get_port.default)();
  return new Promise((resolve2) => {
    const server = app.listen(port, () => {
      const result = `http://localhost:${port}/iframe.html`;
      import_node_logger3.logger.info(`connecting to: ${result}`);
...
var usePuppeteerBrowser = async () => {
  ...
  try {
    return await import_puppeteer_core.default.launch({ args, executablePath: process.env.SB_CHROMIUM_PATH });
  } catch (e) {
    import_node_logger3.logger.info("installing puppeteer...");
    return new Promise((resolve2, reject) => {
      require("child_process").exec(
        `node ${require.resolve(import_path12.default.join("puppeteer-core", "install.js"))}`,
        (error) => error ? reject(error) : resolve2(import_puppeteer_core.default.launch({ args }))
      );
    });
  }
};
async function extract(input, targetPath) {
  if (input && targetPath) {
    ...
    await (0, import_fs_extra8.writeFile)(targetPath, JSON.stringify(data, null, 2));
    await exit();
  } else {
    throw new Error(
...
// src/upgrade.ts
var import_cross_spawn4 = require("cross-spawn");
var import_telemetry2 = require("@storybook/telemetry");
var import_semver14 = __toESM(require("@storybook/semver"));
var import_node_logger4 = require("@storybook/node-logger");
var versionRegex = /(@storybook\/[^@]+)@(\S+)/;
var getStorybookVersion = (line) => {
  if (line.startsWith("npm "))
    return null;
  const match = versionRegex.exec(line);
  if (!match || !import_semver14.default.clean(match[2]))
    return null;
  return {
    package: match[1],
    version: match[2]
...
var warnPackages = (pkgs) => pkgs.forEach((pkg2) => import_node_logger4.logger.warn(`- ${formatPackage(pkg2)}`));
var checkVersionConsistency = () => {
  const lines = (0, import_cross_spawn4.sync)("npm", ["ls"], { stdio: "pipe", shell: true }).output.toString().split("\n");
  ...
  if (!storybookPackages.length) {
    import_node_logger4.logger.warn("No storybook core packages found.");
    import_node_logger4.logger.warn(`'npm ls | grep storybook' can show if multiple versions are installed.`);
    return;
  }
  storybookPackages.sort((a, b) => import_semver14.default.rcompare(a.version, b.version));
  ...
  if (outdated.length > 0) {
    import_node_logger4.logger.warn(
      `Found ${outdated.length} outdated packages (relative to '${formatPackage(
        storybookPackages[0]
      )}')`
    );
    import_node_logger4.logger.warn("Please make sure your packages are updated to ensure a consistent experience.");
    warnPackages(outdated);
  }
  deprecatedPackages.forEach(({ minVersion: minVersion2, url, deprecations }) => {
    if (import_semver14.default.gte(latestVersion, minVersion2)) {
      const deprecated = storybookPackages.filter((pkg2) => deprecations.includes(pkg2.package));
      if (deprecated.length > 0) {
        import_node_logger4.logger.warn(`Found ${deprecated.length} deprecated packages since ${minVersion2}`);
        import_node_logger4.logger.warn(`See ${url}`);
...
var addExtraFlags = (extraFlags, flags, { dependencies, devDependencies }) => {
  return Object.entries(extraFlags).reduce(
    (acc, entry) => {
      ...
      if (pkgVersion && import_semver14.default.satisfies(import_semver14.default.coerce(pkgVersion), specifier)) {
        return [...acc, ...extra];
      }
      return acc;
    },
...
var upgrade = async ({
  ...
  if (!options.disableTelemetry) {
    (0, import_telemetry2.telemetry)("upgrade", { prerelease });
  }
  let flags = [];
  if (!dryRun)
    flags.push("--upgrade");
  ...
  const check = (0, import_cross_spawn4.sync)("npx", ["npm-check-updates@latest", "/storybook/", ...flags], {
    stdio: "pipe",
    shell: true
  }).output.toString();
  import_node_logger4.logger.info(check);
  if (!dryRun) {
...
`);
  ...
  return new Promise((resolve2, reject) => {
    ...
    const child = import_shelljs2.default.exec(command2, {
      ...defaultOptions2,
      ...options,
      async: true,
      silent: false
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        ...
        logger16.error(import_chalk16.default.red(`An error occurred while executing: \`${command2}\``));
...
var addPackageResolutions = async ({ cwd }) => {
  ...
  const packageJsonPath = import_path13.default.join(cwd, "package.json");
  const packageJson = await (0, import_fs_extra9.readJSON)(packageJsonPath);
  packageJson.resolutions = versions_default;
  await (0, import_fs_extra9.writeJSON)(packageJsonPath, packageJson, { spaces: 2 });
};
var addLocalPackageResolutions = async ({ cwd }) => {
  logger16.info(`\u{1F522} Adding package resolutions:`);
  const packageJsonPath = import_path13.default.join(cwd, "package.json");
  const packageJson = await (0, import_fs_extra9.readJSON)(packageJsonPath);
  const workspaceDir = import_path13.default.join(__dirname, "..", "..", "..", "..", "..");
  const { stdout } = await (0, import_execa.command)("yarn workspaces list --json", { cwd: workspaceDir });
  ...
  packageJson.resolutions = Object.keys(versions_default).reduce((acc, key) => {
    return {
      ...acc,
      [key]: import_path13.default.join(workspaceDir, workspaces.find((item) => item.name === key).location)
    };
  }, {});
  await (0, import_fs_extra9.writeJSON)(packageJsonPath, packageJson, { spaces: 2 });
};
...
var addAdditionalFiles = async ({ additionalFiles, cwd }) => {
  ...
  await Promise.all(
    additionalFiles.map(async (file) => {
      await (0, import_fs_extra9.outputFile)(import_path13.default.resolve(cwd, file.path), file.contents, { encoding: "UTF-8" });
    })
  );
};
...
var addRequiredDeps = async ({ cwd, additionalDeps }) => {
  import_shelljs2.default.rm("-f", import_path13.default.join(cwd, "package-lock.json"), import_path13.default.join(cwd, "yarn.lock"));
...
var addTypescript = async ({ cwd }) => {
  ...
  try {
    ...
    const tsConfigJsonPath = import_path13.default.resolve(cwd, "tsconfig.json");
    await (0, import_fs_extra9.writeJSON)(tsConfigJsonPath, tsConfig, { encoding: "utf8", spaces: 2 });
  } catch (e) {
...
var registryUrlNPM = (url) => {
  ...
  return exec(`npm ${args.join(" ")}`, { cwd: import_path13.default.join(process.cwd(), "..") });
};
var registryUrlYarn = (url) => {
  const args = ["config", "set", "npmRegistryServer", url];
  return exec(`yarn ${args.join(" ")}`, { cwd: import_path13.default.join(__dirname, "..") });
};
var createAndInit = async (cwd, { name, version, ...rest }, { e2e, pnp, local, registry }) => {
  const options = {
    name,
    version,
    appName: import_path13.default.basename(cwd),
    creationPath: import_path13.default.join(cwd, ".."),
    cwd,
    e2e,
    pnp,
    ...rest
...
var repro = async ({
  ...
  logger17.info(
    (0, import_boxen2.default)(
      import_ts_dedent13.dedent`
        🤗 Welcome to ${import_chalk17.default.yellow("sb repro")}! 🤗 

        Create a ${import_chalk17.default.green("new project")} to minimally reproduce Storybook issues.
  ...
  if (!selectedTemplate && !generator21) {
    if (!selectedRenderer) {
      const { renderer: rendererOpt } = await (0, import_prompts4.default)({
        type: "select",
    ...
    selectedTemplate = (await (0, import_prompts4.default)({
      type: "select",
  ...
  if (!selectedDirectory) {
    const { directory } = await (0, import_prompts4.default)({
      type: "text",
      message: "Enter the output directory",
      name: "directory",
      initial: selectedConfig.name,
      validate: (directoryName) => import_fs6.default.existsSync(directoryName) ? `${directoryName} already exists. Please choose another name.` : true
    });
    selectedDirectory = directory;
  }
  try {
    const cwd = import_path14.default.isAbsolute(selectedDirectory) ? selectedDirectory : import_path14.default.join(process.cwd(), selectedDirectory);
    ...
    logger17.info(
      (0, import_boxen2.default)(
        ...
        ${import_chalk17.default.yellow(`cd ${selectedDirectory}`)}
        ${import_chalk17.default.yellow(`yarn storybook`)}

        Once you've recreated the problem you're experiencing, please:
        
        1. Document any additional steps in ${import_chalk17.default.cyan("README.md")}
...
var reproNext = async ({
    ...
    logger18.info(
      (0, import_boxen3.default)(
        import_ts_dedent14.dedent`
          ...
          After filtering all the templates with "${import_chalk18.default.yellow(
          filterValue
        )}", we found no results. Please try again with a different filter.

          Available templates:
          ${keys.map((key) => import_chalk18.default.blue`- ${key}`).join("\n")}
          `.trim(),
        { borderStyle: "round", padding: 1, borderColor: "#F1618C" }
      )
    );
  ...
  if (choices.length === 1) {
    ...
    logger18.info(
      (0, import_boxen3.default)(
        import_ts_dedent14.dedent`
          🤗 Welcome to ${import_chalk18.default.yellow("sb repro NEXT")}! 🤗 
  
          Create a ${import_chalk18.default.green("new project")} to minimally reproduce Storybook issues.
  ...
  if (selectedDirectory && (0, import_fs_extra10.existsSync)(`${selectedDirectory}`)) {
    logger18.info(`\u26A0\uFE0F  ${selectedDirectory} already exists! Overwriting...`);
  }
  if (!selectedDirectory) {
    const { directory } = await (0, import_prompts5.default)({
      type: "text",
      message: "Enter the output directory",
      name: "directory",
      initial: outputDirectoryName,
      validate: async (directoryName) => (0, import_fs_extra10.existsSync)(directoryName) ? `${directoryName} already exists. Please choose another name.` : true
    });
    selectedDirectory = directory;
  }
  try {
    const templateDestination = import_path15.default.isAbsolute(selectedDirectory) ? selectedDirectory : import_path15.default.join(process.cwd(), selectedDirectory);
    ...
    const initMessage = init ? import_chalk18.default.yellow(`yarn storybook`) : `Recreate your setup, then ${import_chalk18.default.yellow(`run npx storybook init`)}`;
    logger18.info(
      (0, import_boxen3.default)(
        import_ts_dedent14.dedent`
        🎉 Your Storybook reproduction project is ready to use! 🎉

        ${import_chalk18.default.yellow(`cd ${selectedDirectory}`)}
        ${initMessage}

        Once you've recreated the problem you're experiencing, please:
        
        1. Document any additional steps in ${import_chalk18.default.cyan("README.md")}
        2. Publish the repository to github
        3. Link to the repro repository in your issue
...
async function promptSelectedTemplate(choices) {
  const { template } = await (0, import_prompts5.default)({
    type: "select",
...
var import_cross_spawn5 = require("cross-spawn");
var import_node_logger5 = require("@storybook/node-logger");
var link = async ({ target, local, start }) => {
  const storybookDir = process.cwd();
  try {
    const packageJson = JSON.parse(import_fs_extra11.default.readFileSync("package.json", "utf8"));
  ...
  let reproDir = target;
  let reproName = import_path16.default.basename(target);
  if (!local) {
    const reprosDir = import_path16.default.join(storybookDir, "../storybook-repros");
    import_node_logger5.logger.info(`Ensuring directory ${reprosDir}`);
    import_fs_extra11.default.ensureDirSync(reprosDir);
    import_node_logger5.logger.info(`Cloning ${target}`);
    await exec(`git clone ${target}`, { cwd: reprosDir });
    reproName = import_path16.default.basename(target, import_path16.default.extname(target));
    reproDir = import_path16.default.join(reprosDir, reproName);
  }
  const version = (0, import_cross_spawn5.sync)("yarn", ["--version"], {
    cwd: reproDir,
    stdio: "pipe",
    shell: true
  }).stdout.toString();
  if (!/^[23]\./.test(version)) {
    import_node_logger5.logger.warn(`\u{1F6A8} Expected yarn 2 or 3 in ${reproDir}!`);
    import_node_logger5.logger.warn("");
    import_node_logger5.logger.warn("Please set it up with `yarn set version berry`,");
    import_node_logger5.logger.warn(`then link '${reproDir}' with the '--local' flag.`);
    return;
  }
  import_node_logger5.logger.info(`Linking ${reproDir}`);
  await exec(`yarn link --all ${storybookDir}`, { cwd: reproDir });
  import_node_logger5.logger.info(`Installing ${reproName}`);
  await exec(`yarn install`, { cwd: reproDir });
  import_node_logger5.logger.info(
  ...
  if (start) {
    import_node_logger5.logger.info(`Running ${reproName} storybook`);
...
var import_node_logger6 = require("@storybook/node-logger");
var import_core_server = require("@storybook/core-server");
var import_core_common8 = require("@storybook/core-common");
var dev = async (cliOptions) => {
  var _a2;
  process.env.NODE_ENV = process.env.NODE_ENV || "development";
  try {
    await (0, import_core_server.buildDevStandalone)({
      ...cliOptions,
      configDir: cliOptions.configDir || "./.storybook",
      configType: "DEVELOPMENT",
      ignorePreview: !!cliOptions.previewUrl && !cliOptions.forceBuildPreview,
      docsMode: !!cliOptions.docs,
      cache: import_core_common8.cache,
      packageJson: (0, import_read_pkg_up.sync)({ cwd: __dirname }).packageJson
    });
  } catch (error) {
    import_node_logger6.instance.heading = "";
    if (error instanceof Error) {
      if (error.error) {
        import_node_logger6.logger.error(error.error);
      } else if (error.stats && error.stats.compilation.errors) {
        error.stats.compilation.errors.forEach((e) => import_node_logger6.logger.plain(e));
      } else {
        import_node_logger6.logger.error(error);
      }
    } else if ((_a2 = error.compilation) == null ? void 0 : _a2.errors) {
      error.compilation.errors.forEach((e) => import_node_logger6.logger.plain(e));
    }
    import_node_logger6.logger.line();
    import_node_logger6.logger.warn(
      error.close ? import_ts_dedent15.dedent`
...
// src/build.ts
var import_read_pkg_up2 = require("read-pkg-up");
var import_node_logger7 = require("@storybook/node-logger");
var import_core_server2 = require("@storybook/core-server");
var import_core_common9 = require("@storybook/core-common");
var build = async (cliOptions) => {
  try {
    await (0, import_core_server2.buildStaticStandalone)({
      ...cliOptions,
      configDir: cliOptions.configDir || "./.storybook",
      ...
      cache: import_core_common9.cache,
      packageJson: (0, import_read_pkg_up2.sync)({ cwd: __dirname }).packageJson
    });
  } catch (e) {
    import_node_logger7.logger.error(e);
    process.exit(1);
  }
};
...
// src/generate.ts
var pkg = (0, import_read_pkg_up3.sync)({ cwd: __dirname }).packageJson;
var consoleLogger = console;
import_commander.default.option(
  "--disable-telemetry",
  "disable sending telemetry data",
  process.env.STORYBOOK_DISABLE_TELEMETRY && process.env.STORYBOOK_DISABLE_TELEMETRY !== "false"
);
import_commander.default.option("--enable-crash-reports", "enable sending crash reports to telemetry data");
import_commander.default.command("init").description("Initialize Storybook into your project.").option("-f --force", "Force add Storybook").option("-s --skip-install", "Skip installing deps").option("-N --use-npm", "Use npm to install deps").option("--use-pnp", "Enable pnp mode").option("-p --parser <babel | babylon | flow | ts | tsx>", "jscodeshift parser").option("-t --type <type>", "Add Storybook for a specific project type").option("-y --yes", "Answer yes to all prompts").option("-b --builder <builder>", "Builder library").option("-l --linkable", "Prepare installation for link (contributor helper)").action(
  (options) => initiate(options, pkg).catch((err) => {
    import_node_logger8.logger.error(err);
    process.exit(1);
  })
);
import_commander.default.command("add <addon>").description("Add an addon to your Storybook").option("-N --use-npm", "Use NPM to build the Storybook server").option("-s --skip-postinstall", "Skip package specific postinstall config modifications").action((addonName, options) => add(addonName, options));
import_commander.default.command("babelrc").description("generate the default storybook babel config into your current working directory").action(() => generateStorybookBabelConfigInCWD());
import_commander.default.command("upgrade").description("Upgrade your Storybook packages to the latest").option("-N --use-npm", "Use NPM to build the Storybook server").option("-y --yes", "Skip prompting the user").option("-n --dry-run", "Only check for upgrades, do not install").option("-p --prerelease", "Upgrade to the pre-release packages").option("-s --skip-check", "Skip postinstall version and automigration checks").action((options) => upgrade(options));
import_commander.default.command("info").description("Prints debugging information about the local environment").action(() => {
  consoleLogger.log(import_chalk19.default.bold("\nEnvironment Info:"));
...
import_commander.default.command("migrate [migration]").description("Run a Storybook codemod migration on your source files").option("-l --list", "List available migrations").option("-g --glob <glob>", "Glob for files upon which to apply the migration", "**/*.js").option("-p --parser <babel | babylon | flow | ts | tsx>", "jscodeshift parser").option(
  "-n --dry-run",
...
import_commander.default.command("repro-next [filterValue]").description("Create a reproduction from a set of possible templates").option("-o --output <outDir>", "Define an output directory").option("-b --branch <branch>", "Define the branch to degit from", "next").option("--no-init", "Whether to download a template without an initialized Storybook", false).action(
  (filterValue, options) => reproNext({ filterValue, ...options }).catch((e) => {
    import_node_logger8.logger.error(e);
    process.exit(1);
  })
);
import_commander.default.command("link <repo-url-or-directory>").description("Pull down a repro from a URL (or a local directory), link it, and run storybook").option("--local", "Link a local directory already in your file system").option("--no-start", "Start the storybook", true).action(
  (target, { local, start }) => link({ target, local, start }).catch((e) => {
    import_node_logger8.logger.error(e);
    process.exit(1);
  })
);
import_commander.default.command("automigrate [fixId]").description("Check storybook for known problems or migrations and apply fixes").option("-y --yes", "Skip prompting the user").option("-n --dry-run", "Only check for fixes, do not actually run them").action(
  (fixId, options) => automigrate({ fixId, ...options }).catch((e) => {
    import_node_logger8.logger.error(e);
    process.exit(1);
  })
);
import_commander.default.command("dev").option("-p, --port <number>", "Port to run Storybook", (str) => parseInt(str, 10)).option("-h, --host <string>", "Host to run Storybook").option("-s, --static-dir <dir-names>", "Directory where to load static files from", parseList).option("-c, --config-dir <dir-name>", "Directory where to load Storybook configurations from").option(
  "--https",
  ...
  import_node_logger8.logger.setLevel(import_commander.default.loglevel);
  consoleLogger.log(import_chalk19.default.bold(`${pkg.name} v${pkg.version}`) + import_chalk19.default.reset("\n"));
  getEnvConfig(import_commander.default, {
    port: "SBCONFIG_PORT",
  ...
  if (typeof import_commander.default.port === "string" && import_commander.default.port.length > 0) {
    import_commander.default.port = parseInt(import_commander.default.port, 10);
  }
  dev({ ...options, packageJson: pkg });
});
import_commander.default.command("build").option("-s, --static-dir <dir-names>", "Directory where to load static files from", parseList).option("-o, --output-dir <dir-name>", "Directory where to store built files").option("-c, --config-dir <dir-name>", "Directory where to load Storybook configurations from").option("--quiet", "Suppress verbose build output").option("--loglevel <level>", "Control level of logging during build").option("--debug-webpack", "Display final webpack configurations for debugging purposes").option("--webpack-stats-json [directory]", "Write Webpack Stats JSON to disk").option(
  "--preview-url <string>",
  "Disables the default storybook preview and lets your use your own"
  ...
  import_node_logger8.logger.setLevel(import_commander.default.loglevel);
  consoleLogger.log(import_chalk19.default.bold(`${pkg.name} v${pkg.version}
`));
  getEnvConfig(import_commander.default, {
    staticDir: "SBCONFIG_STATIC_DIR",
...
import_commander.default.on("command:*", ([invalidCmd]) => {
  consoleLogger.error(
    " Invalid command: %s.\n See --help for a list of available commands.",
    invalidCmd
  );
  const availableCommands = import_commander.default.commands.map((cmd) => cmd._name);
  const suggestion = availableCommands.find((cmd) => (0, import_leven.default)(cmd, invalidCmd) < 3);
  if (suggestion) {
    consoleLogger.info(`
...
import_commander.default.usage("<command> [options]").version(pkg.version).parse(process.argv);
