...
var babelParse = (code) => (0, import_parser.parse)(code, {
  sourceType: "module",
  plugins: [
    "jsx",
    "typescript",
...
var CsfFile = class {
    ...
    declaration.properties.forEach((p) => {
      if (t.isIdentifier(p.key)) {
        ...
        if (p.key.name === "title") {
          ...
          const { code } = (0, import_generator.default)(p.value, {});
          meta.component = code;
  ...
  parse() {
    const self = this;
    (0, import_traverse.default)(this._ast, {
    ...
    self._stories = entries.reduce((acc, [key, story]) => {
      if ((0, import_csf.isExportStory)(key, self._meta)) {
        const id = (0, import_csf.toId)(self._meta.id || self._meta.title, (0, import_csf.storyNameFromExport)(key));
...
var formatCsf = (csf) => {
  const { code } = (0, import_generator.default)(csf._ast, {});
  return code;
};
var readCsf = async (fileName, options) => {
  const code = (await import_fs_extra.default.readFile(fileName, "utf-8")).toString();
...
var writeCsf = async (csf, fileName) => {
  ...
  await import_fs_extra.default.writeFile(fileName, await formatCsf(csf));
};
...
var ConfigFile = class {
  ...
  parse() {
    const self = this;
    (0, import_traverse2.default)(this._ast, {
      ExportNamedDeclaration: {
        enter({ node, parent }) {
  ...
  getFieldValue(path) {
    ...
    if (node) {
      const { code } = (0, import_generator2.default)(node, {});
  ...
  setFieldValue(path, value) {
    ...
    let valueNode;
    if (quotes === "single") {
      const { code } = (0, import_generator2.default)(t2.valueToNode(value), { jsescOption: { quotes } });
      const program = babelParse(`const __x = ${code}`);
      (0, import_traverse2.default)(program, {
        VariableDeclaration: {
          enter({ node }) {
...
var formatConfig = (config) => {
  const { code } = (0, import_generator2.default)(config._ast, {});
  return code;
};
var readConfig = async (fileName) => {
  const code = (await import_fs_extra2.default.readFile(fileName, "utf-8")).toString();
...
var writeConfig = async (config, fileName) => {
  ...
  await import_fs_extra2.default.writeFile(fname, await formatConfig(config));
};

// src/getStorySortParameter.ts
var t3 = __toESM(require("@babel/types"));
var import_traverse3 = __toESM(require("@babel/traverse"));
var import_generator3 = __toESM(require("@babel/generator"));
...
var unsupported = (unexpectedVar, isError) => {
  const message = import_ts_dedent2.dedent`
...
var getStorySortParameter = (previewCode) => {
  ...
  if (t3.isArrowFunctionExpression(storySort)) {
    const { code: sortCode } = (0, import_generator3.default)(storySort, {});
