...
function getDocumentWidthAndHeight() {
  const container = import_global.default.document.documentElement;
...
function createCanvas() {
  const canvas = import_global.default.document.createElement("canvas");
...
function setCanvasWidthAndHeight(canvas, context, { width, height }) {
  ...
  const scale = import_global.default.window.devicePixelRatio;
...
function measureElement(element) {
  const style = import_global2.default.getComputedStyle(element);
...
var deepElementFromPoint = (x, y) => {
  const element = import_global3.default.document.elementFromPoint(x, y);
